...
The energetic deposits of particles need to be clustered to obtain the
complete response in our detector. This is because the process
...
the mathematical implementation of this algorithm
is outlined in Ref.~\cite{fastjet_timing}, and the computational time
scales as $O(N^2)$ or  $O(N \ln{N}$, where $N$ is the number
...
parallelization of the kNN algorithm itself to make
...
%large number of infinitesimally small inputs called ``ghosts''
...
real-world application of LHC data, specifically by reducing the
computational time for $k$-nearest-neighbor-like numerical
kernels. The investigators of this proposal have extensive experience
in the various aspects of the problem, and the synergistic application
of this experience is expected to attain considerable improvements in
this avenue. 
